<script src="//code.jquery.com/jquery-1.11.1.min.js"></script> 
<script src="//maxcdn.bootstrapcdn.com/bootstrap/3.3.0/js/bootstrap.min.js"></script>

<?php include_once '../Lib/helpers.php'; ?>

<div class="card shadow-lg mt-5" id="card_red_man">
    <div class="card-header bg-secondary text-white text-center">
        <h3 class="display-6 mb-0">Consultar Señalizacion nueva</h3>
    </div>

    <div class="card-body">

    <div class="row mb-3">
        <div class="col-md-4">
            <input type="text" name="buscarSenializacion" id="buscarSenializacion" class="form-control" placeholder="Buscar por nombre o correo" 
                data-url='<?php echo getUrl("Senializacion", "Senializacion", "buscar", false, "ajax"); ?>'>
        </div>
    </div>
    <div class="table-responsive">
        <table class="table table-hover table-striped">
            <thead class="table-secondary">
                <tr>
                    <th>Id</th>
                    <th>Fecha</th>
                    <th>Observacion</th>
                    <th>Orientacion</th>
                    <th>Categoria</th>
                    <th>Tipo de Señalizacion</th>
                    <th>Estado</th>

                </tr>
            </thead>
            <tbody>
            <?php
            foreach($senializacion as $sen){
                $clase="";
                $texto="";
                echo "<tr>";
                echo "<td>".$senializacion['id_senializacion_vial_nueva']."</td>";
                echo "<td>".$senializacion['fecha']."</td>";
                echo "<td>".$senializacion['descripcion']."</td>";
                echo "<td>".$senializacion['nombre_o']."</td>";
                echo "<td>".$senializacion['nombre_c_s']."</td>";
                echo "<td>".$senializacion['tipo_senializacion']."</td>";
                echo "<td>";
                echo"<form action='getUrl('Senializacion', 'Senializacion', 'postUpdateStatus');' method='post' class='mt-4'>";
                echo "<select class='form-select' name='id' id='id'>";
                echo "<option disabled selected>".($senializacion['edescripcion'])."</option>";
                foreach ($estado as $est) {
                echo "<option value='".($est['id_estado'])."'>".($est['nombre_estado'])."</option>";
                }
                echo "</select>";
                echo"<br>";
                echo "<button type='submit' class='btn btn-dark'>Enviar</button>";
                echo "</form>";
                echo "</td>";
                


                echo "</tr>";
            }

            ?>
            </tbody>
        </table>
    </div>
    </div>
</div>